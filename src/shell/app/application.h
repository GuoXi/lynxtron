...
#include "shell/app/window_list_observer.h"
...
class Application : private WindowListObserver {
 public:
  Application();
...
// TODO(Guo Xi): remove the macro defines
  ...
  // one from app's name.
  // The returned string managed by Application, and should not be modified.
  ...
  void AddObserver(ApplicationObserver* obs);
  void RemoveObserver(ApplicationObserver* obs);
 ...
 private:
  ...
  // Observers of the application.
  base::ObserverList<ApplicationObserver> observers_;
